...
class Signing:
    ...
    def prepare(self):
        ...
        # Empty P12 password is safe only inside this 0700 ephemeral runner directory.
        run("openssl", "pkcs12", "-export", "-inkey", str(key), "-in", str(pem_path), "-out", str(p12), "-passout", "pass:", "-keypbe", "PBE-SHA1-3DES", "-certpbe", "PBE-SHA1-3DES", "-macalg", "sha1")
        run("security", "import", str(p12), "-k", str(keychain), "-P", "", "-T", "/usr/bin/codesign", "-T", "/usr/bin/security")
    ...
    def cleanup(self):
        errors = []
        for state_key, resource in [("profile_id", "profiles"), ("certificate_id", "certificates")]:
...
def main():
    ...
    parser.add_argument('command', choices=['metadata', 'prepare', 'cleanup'])
